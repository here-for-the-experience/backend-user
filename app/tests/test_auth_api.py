...
#     response = client.put("/update", data = { "password" : new_password }, headers = { "Authorization" : f"Bearer {token['access_token']}" })
...
# def test_cities() :
#     response = client.get(ENDPOINT + "/city")
#     assert response.status_code == 200
    
# def test_center() :
#     response = client.get(ENDPOINT + "/center")
#     assert response.status_code == 200
